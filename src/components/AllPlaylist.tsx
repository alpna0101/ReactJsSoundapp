...
React.useEffect(() => {
	...
	getsongs(props.match.params.id, activePage);
...
if(playsongs.length!=0){
	...
	//    if (currentSong ==playsongs[key]['song_file']){
...
const getplaylistdata =(playlist_id:any)=>{
	axios.get(`${config.apiUrl}/genres/getGenre/`+playlist_id)
    .then(async (response) => {
       
		setplaylist_name(response.data.genre[0].name)
		
		setplaylist_des(response.data.genre[0].description)
		setplaylist_img(response.data.genre[0].image)
...
//  $('#myModalplay').css("display","block");
    ...
    axios.get(`${config.apiUrl}/songs/getSong/`+playlist_id+`/`+active_page).then(response => response.data)
    .then((data) => {
		// console.log(data);
		setitemPerPage(data.getSongs)
		settotaldata(data.totalSongs)
...
if(songs.length!=0){
	myplaylists = Object.keys(songs).map(function(key: any,value:any){
	 ...
	 <a data-dismiss="modal" data-image={songs[key]['image']} onClick={ (e:any) => changePlayList(songs[key]['_id']) } data-name={songs[key]['name']} data-des={songs[key]['description']} id={songs[key]['_id']}>
...
<div className="modal right fade playlist_pop" id="personl_playlist"  role="dialog" aria-labelledby="myplalists">
		<div className="modal-dialog" role="document">
			...
			<div className="summry_btns">
			<a  href="/" style={ { width:`100%`} } className="btn">	Back to home</a>
