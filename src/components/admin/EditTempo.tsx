...
const submit = (values: any,Keyname:any,Keyid:any) => {
  ...
  if (validateForm(Keyname)) {
    ...
    const headers = {
     ...
     axios.put(`${config.apiUrl}/tempos/editTempo`, {
      tempo_name: Keyname,
      id: Keyid
...
const Login = (props: any) => {
  ...
  const getkeywords = () =>{
    axios.get(`${config.apiUrl}/tempos/getTempo/`+id)
    .then(async (response) => {
    
      
      
      setkeyname(response.data.Tempo[0].tempo_name)
      setkeyid(response.data.Tempo[0]._id)
    ...
    <div className="user_table user-form">
         ...
         (() => {
          if (Keyname!= undefined)
             ...
             <label>Name</label>
             <input type="text" placeholder="Enter Tempo Name" name="keyname" value={Keyname}    onChange={handleChangeekey} className="form-control" id="email" />
