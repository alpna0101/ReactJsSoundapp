...
const submit = (values: any,Keyname:any,Keyid:any) => {
  ...
  return sleep(1000).then(() => {
    ...
    const headers = {
     ...
     axios.put(`${config.apiUrl}/keys/editKey`, {
      key_name: Keyname,
      id: Keyid
      ...
      alert(response.data.message);
       window.location.href = `${config.appurl}/admin/key`; ; 
       }
    })
  }
})
...
const renderField = ({ input, label, type,value, meta: { touched, error } }: fieldInterface) => (
  ...
  const validateForm= (data:any) => {
    ...
    console.log("fields======",fields.trim(),"fields======");
    if (fields.trim() == "") {
      console.log("fields======",fields.trim(),"fields======");
      formIsValid = false;
...
const Login = (props: any) => {
  ...
  const getkeywords = () =>{
    axios.get(`${config.apiUrl}/keys/getKey/`+id)
    .then(async (response) => {
    
      
      setkeywords(response.data.key);
      setkeyname(response.data.key[0].key_name)
      setkeyid(response.data.key[0]._id)
    ...
    <div className="user_table user-form">
         ...
         (() => {
          if (Keyname!= undefined)
             ...
             <input type="text" placeholder="Enter Key Name" name="keyname" value={Keyname}    onChange={handleChangeekey} className="form-control" id="email" />
