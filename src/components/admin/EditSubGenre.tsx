...
const submit = (values: any,Keyname:any,Keyid:any) => {
  ...
  return sleep(1000).then(() => {
    ...
    const headers = {
     ...
     axios.put(`${config.apiUrl}/subgenres/editSubGenre`, {
        name: Keyname,
      id: Keyid,
     type:'S'
...
const Login = (props: any) => {
  ...
  const getkeywords = () =>{
    ...
    axios.get(`${config.apiUrl}/subgenres/getSubGenreById/`+id)
    .then(async (response) => {

      console.log(response.data)
    //  setkeywords(response.data.key);
      setkeyname(response.data.subgenre[0].name)
      setkeyid(response.data.subgenre[0]._id)
    ...
    <div className="user_table user-form">
         ...
         <div className="user-form">
          ...
          return <div className="col-md-8">
            <label>Name</label>
             <input type="text" placeholder="Edit Name" name="keyname" value={Keyname}    onChange={handleChangeekey} className="form-control" id="email" />
