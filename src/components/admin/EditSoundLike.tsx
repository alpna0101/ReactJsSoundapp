...
const submit = (values: any,Keyname:any,Keyid:any) => {
  ...
  if (validateForm(Keyname)) {
    ...
    const headers = {
     ...
     axios.put(`${config.apiUrl}/sounds/editSound`, {
        sound_name: Keyname,
      id: Keyid
      ...
      if(response.data.status==false){
        ...
        alert(response.data.message);
       window.location.href = `${config.appurl}/admin/soundlike`; ; 
       }
    })
  }
})
...
const renderField = ({ input, label, type,value, meta: { touched, error } }: fieldInterface) => (
  <div className="form-group">
      <label>{label}</label>
        ...
        <input {...input} placeholder={label} type={type} className="form-control" value={value}/>
  ...
  const validateForm= (data:any) => {
    ...
    if (!fields) {
      formIsValid = false;
      throw new SubmissionError({ keyname: 'Name field required', _error: 'Name field required' })
...
const Login = (props: any) => {
  ...
  const getkeywords = () =>{
    axios.get(`${config.apiUrl}/sounds/getSound/`+id)
    .then(async (response) => {
    
      
      
      setkeyname(response.data.sound[0].sound_name)
      setkeyid(response.data.sound[0]._id)
    ...
    <div className="user_table user-form">
         ...
         (() => {
          if (Keyname!= undefined)
             ...
             <label>Name</label>
             <input type="text" placeholder="Enter Sound Like Name" name="keyname" value={Keyname}    onChange={handleChangeekey} className="form-control" id="email" />
