...
const submit = (values: any,Keyname:any,Keyid:any) => {
  ...
  return sleep(1000).then(() => {
    ...
    const headers = {
     ...
     axios.put(`${config.apiUrl}/producers/editProducer`, {
        producer_name: Keyname,
      id: Keyid
...
const Login = (props: any) => {
  ...
  const getpro = () =>{
    axios.get(`${config.apiUrl}/producers/getProducer/`+id,config1)
    .then(async (response) => {
        console.log(response.data.producer)
       setkeyname(response.data.producer[0].producer_name)
      setkeyid(response.data.producer[0]._id)
